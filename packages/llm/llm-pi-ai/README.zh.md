...
- **`headers` 可以携带 redactor 永远看不到的凭据**——profile 解析会拒绝 Fetch 无法表示的名称与值，但该字典仍是纯字符串；以 `apiKeyEnv` 引用存储凭据。值支持请求时模板 `${sessionId}`（本对话的稳定 SessionId）、`${provider}` 与 `${model}`；未知 token 保持字面量，模型发现保持静态标头，因为探测没有会话。
